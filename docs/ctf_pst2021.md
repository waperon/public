...
Stor takk til flere folk for god hjelp og samarbeid. Ingen nevnt, ingen glemt. Dere vet hvem dere er 🙏
...
fun main() {
    ...
    File("verksted_npst.txt").readLines().drop(1).forEach {
        ...
        inv.add(Inventory(
            indeks = split[0].toInt(), 
            hylleplass = split[1].trim().toInt(radix = 16), 
            utstyr = split[2], 
            id = split[3].toInt())
        )
    }
